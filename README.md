...
- [설계 목표와 판단 기준](#설계-목표와-판단-기준)
- [전체 아키텍처](#전체-아키텍처)
- [상태 모델 설계 의도](#상태-모델-설계-의도)
- [외부 시스템 연동 방식 및 선택 이유](#외부-시스템-연동-방식-및-선택-이유)
- [실패 처리 전략](#실패-처리-전략)
- [동시 요청 발생 시 고려 사항](#동시-요청-발생-시-고려-사항)
- [트래픽 증가 시 병목 가능 지점](#트래픽-증가-시-병목-가능-지점)
- [처리 보장 모델과 재시작 시 동작](#처리-보장-모델과-재시작-시-동작)
- [테스트 전략](#테스트-전략)
- [실행 방법](#실행-방법)
- [한계와 확장 포인트](#한계와-확장-포인트)
...
허용 전이에 포함되지 않은 나머지 전이는 모두 금지합니다. 특히 아래 전이는 명시적으로 막고 있습니다.

- `RECEIVED -> RUNNING`
- `RECEIVED -> SUCCEEDED`, `RECEIVED -> FAILED`
- `QUEUED -> SUCCEEDED`, `QUEUED -> FAILED`
- `SUCCEEDED -> *`
- `FAILED -> *`
- `SUCCEEDED <-> FAILED`

이 제약을 둔 이유는 상태의 의미를 흐리지 않기 위해서입니다. 예를 들어 `QUEUED`에서 바로 `SUCCEEDED`로 가게 두면 실제 외부 실행이 있었는지, 중간 상태가 생략된 것인지 구분할 수 없습니다. `SUCCEEDED`, `FAILED`에서 다시 다른 상태로 움직이게 두면 재시도나 복구 시점마다 결과 일관성이 깨질 수 있습니다. 그래서 실행 전이는 단계별로 제한하고, final state는 불변으로 고정했습니다.
